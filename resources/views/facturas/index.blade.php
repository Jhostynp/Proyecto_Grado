...
@foreach($facturas as $fac)
			...
			<tr>
				...
				<td style="text-align:center;">{{$fac->fac_fecha}}</td>
      			...
      			<td style="text-align:center;">{{$fac->fac_total}}$</td>
				...
				<td style="text-align:center;">{{$fac->fac_estado}}</td>
...
<a href="{{route('facturas.edit',$fac->fac_id)}}" class="btn btn-success rounded float-right">
 ...
 &nbsp; &nbsp; &nbsp;
  <form action="{{route('facturas.destroy',$fac->fac_id)}}" method="POST" onsubmit="return confirm ('Desea eliminar esta categoria?') ">
  	@csrf
